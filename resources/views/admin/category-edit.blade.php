            ...
            <div class="wg-box">
                <form class="form-new-product form-style-1" action="{{ route('admin.category.update') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <fieldset class="name">
                        ...
                        <textarea class="flex-grow" placeholder="Category description" name="description" rows="4" aria-required="true">{{ $category->description }}</textarea>
                    </fieldset>
                    @error('description')
                    <span class="alert alert-danger text-center">{{ $message }}</span>
                    @enderror
                    ...
                    <fieldset>
                        <div class="body-title">Upload Category Image <span class="tf-color-1">*</span></div>
                        <div class="upload-container">
                            @if($category->image)
                            <div class="upload-preview" id="imagePreview">
                                <div class="no-preview cursor-pointer" id="noPreviewText">
                                    <img id="previewImage" src="{{ asset('uploads/categories') }}/{{ $category->image }}" alt=" {{ $category->name }}">
                                </div>
                            </div>
                            @endif
