                                ...
                                <tr>
                                    ...
                                    <td class="px-4 py-2">
                                        <div class="frame w-full h-full flex-shrink-0 mr-4 p-2 border-4 border-gold rounded-lg shadow-lg">
                                            <img src="{{ asset('uploads/categories/' . $category->image) }}" alt="{{ $category->name }}" class="w-full h-full object-cover rounded">
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 py-2">
                                        ...
                                        <div class="name flex justify-center items-center text-center h-full">
                                            <a href="#" class="body-title-2 font-semibold">{{ $category->name }}</a>
                                        </div>
                                    ...
                                    <!-- Brand Description -->
                                    <td class="px-4 py-2">{{ \Illuminate\Support\Str::words($category->description, 20, '...') }}</td>
                                    ...
                                    <td class="px-4 py-2">
                                        <div class="flex justify-center items-center w-full h-full">
                                            <!-- Edit Action -->
                                            <a href="{{ route('admin.category.edit', [ 'id' => $category->id ]) }}" class="flex justify-center items-center">
                                            ...
                                            <!-- Delete Action -->
                                            <form action="{{ route('admin.category.delete', [ 'id' => $category->id ]) }}" method="POST" style="display: inline-block;" class="delete">
                                                @csrf
                                                @method('DELETE')
