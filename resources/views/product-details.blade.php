            ...
            <div class="row">
                    ...
                    <div class="product-single__media" data-media-type="vertical-thumbnail">
                            ...
                            <div class="swiper-container">
                                <div class="swiper-wrapper">
                                    ...
                                    @foreach(json_decode($product->images, true) as $gim)
                                        <div class="swiper-slide product-single__image-item">
                                            <img loading="lazy" class="h-auto" src="{{ asset('uploads/products/gallery/' . trim($gim)) }}" width="674" height="674" alt="{{ $product->name }}" />
                                            <a data-fancybox="gallery" href="{{ asset('uploads/products/gallery/' . trim($gim)) }}" data-bs-toggle="tooltip" data-bs-placement="left" title="Zoom">
                                                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <use href="#icon_zoom" />
                                                </svg>
                                            </a>
                        ...
                        <div class="product-single__thumbnail">
                            <div class="swiper-container">
                                <div class="swiper-wrapper">
                                    @foreach(json_decode($product->images, true) as $gim)
                                        <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto" src="{{ asset('uploads/products/gallery/' . trim($gim)) }}" width="104" height="104" alt="{{ $product->name }}" /></div>
                                    @endforeach
                                </div>
                            </div>
                        </div>
                ...
                <div class="col-lg-5">
                    ...
                    <div class="product-single__addtolinks">
                        ...
                        @if ($wishlistItem)
                            <form method="POST" action="{{ route('wishlist.remove', ['rowId' => $wishlistItem->rowId]) }}" id="wish-remove-id">
                                @csrf
                                @method('DELETE')
                            </form>
                            <a href="javascript:void(0)" class="menu-link menu-link_us-s add-to-wishlist filled-heart" onclick="document.getElementById('wish-remove-id').submit();">
                        ...
                        <div class="swiper-slide product-card">
                            <div class="pc__img-wrapper">
                                <a href="{{ route('shop.product.details', [ 'product_slug' => $rproduct->slug ]) }}">
                                    <img loading="lazy" src="{{ asset('uploads/products') }}/{{ $rproduct->image }}" width="330" height="400" alt="{{ $rproduct->image }}" class="pc__img">
                                    @foreach(json_decode($rproduct->images, true) as $gim)
                                    <img loading="lazy" src="{{ asset('uploads/products/gallery/' . trim($gim)) }}" width="330" height="400" alt="{{ $rproduct->image }}" class="pc__img pc__img-second">
                                    @endforeach
                                </a>
                                ...
                                <div class="product-card__price d-flex">
                                    <span class="money price">
                                        @if($rproduct->sale_price)
                                            <span style="text-decoration: line-through; color: #999; margin-right: 5px;"> Tk. {{ number_format($rproduct->regular_price, 0) }} </span>
                                            <span style="color: #e63946;">Tk. {{ number_format($rproduct->sale_price, 0) }}</span>
                                        @else
                                            <span style="color: #333;"> Tk. {{ number_format($rproduct->regular_price, 0) }}</span>
                                        @endif
                                    </span>
                                </div>
